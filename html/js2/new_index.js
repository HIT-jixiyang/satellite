$(document).ready(function() {
	...
	//地图初始化
	var getdata=''
	var hrefdata=location.search
	if(hrefdata.length>20){
	$(".newdateinput").val("")
	$("#newtimePicker").val("")
	}
	else {
	hrefdata=hrefdata.replace('?','')
	hrefdata1=hrefdata.slice(0,10)
	hrefdata2=hrefdata.slice(10,15)
	...
	function imageChangeFun() {
        g=g+6;
		if(radioindex==1&&g>120){
			g=6
		}
		else if(radioindex==2&&g>180){
			g=6
		}
		...
		console.log(length)
		//取最大值
		var maxLength = Math.max(lengthSeven);
		...
		$(".navgll").css("background-color", "red");
	...
	$("#newlast").click(function() {
		...
		if(stopOrStart == 2) {
			if(lastInd == 1) {
				...
				if(radioindex==1&&imagesNum<0){
				imagesNum=19;
			}
				if(radioindex==2&&imagesNum<0){
				imagesNum=29;
			}
			var length = allMapObj.length;
    ...
    window.location.href="./index2.html?"+getdata+""
    })
	//更新
	$(".newnavgxbtn").click(function() {	
		stopOrStart = 1;
		g=0;
		var dateStr;
		//获取日期时间
	...
	function dateTimeFun(dateStr) {
		...
		var imageLength = 0;
		var imageUrl2=imageUrl = 'image/model6/${date}/pred';
		picArray2 = getPicArray(year, months, days, hour, minute, imageUrl2, "png", 20, "yyyyMMddHHmm", "yyyyMMddHHmm");
		...
		if(radioindex==1){
		for(i=0;i<10;i++){
		var pp=imageArray[2].url
		pp.push(imageArray[1].url[i])
		
		imageArray[2].url=pp
		}
		}
		else if(radioindex==2){
		imageArray[1].url=picArray2.url
		for(i=0;i<20;i++){
		var pp=imageArray[2].url
		pp.push(imageArray[1].url[i])
	...
	//字符串截取
	function subTimeString(imageUrl,g,pd) {
		...
		var minute = g+parseInt(imageUrl.substring(10, 12));
		console.log(g)
		//截取小时

	    if(g>=0){
		var hour = imageUrl.substring(8, 10)-1;
	    }
        else if(g==60){
		var hour = parseInt(imageUrl.substring(8, 10)-1)+2
		}
